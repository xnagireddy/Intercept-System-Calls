...
//hard coded address of sys_call_table from /boot/System.map
...
asmlinkage int (*real_creat)(const char* __user, umode_t);

asmlinkage int custom_creat(const char* __user file_name, umode_t mode) {
    printk(KERN_DEBUG "interceptor[%d-%s]: creat([%s], [%X])", current->pid, current->comm, file_name, mode);
    return real_creat(file_name, mode);
}
...
asmlinkage int custom_open(const char* __user file_name, int flags, umode_t mode) {
    printk(
        "interceptor: (pid-comm): [%d-%s] open(filename: [%s], flasgs: [%X], mode: [%X])\n",
        current->pid, current->comm, file_name, flags, mode
    );

    if(0 == strcmp(file_name, "/tmp/cannotopen")) {
        printk("### NOT ALLOWED TO OPEN [%s] ###\n", file_name);
        return -EPERM;
    }
...
asmlinkage int custom_openat(int dirfd, const char* __user file_name, int flags, umode_t mode) {
    printk(
        "interceptor[%d-%s] openat(dirfd: [%d], filename: [%s], flasgs: [%X], mode: [%X])\n",
        current->pid, current->comm, dirfd, file_name, flags, mode
    );

    if(0 == strcmp(file_name, "/tmp/cannotopen")) {
        printk(KERN_DEBUG "### NOT ALLOWED TO OPEN [%s] ###\n", file_name);
        return -EPERM;
    }

    return real_openat(dirfd, file_name, flags, mode);
}
    ...
    switch(cmd) {
        case IOCTL_PATCH_TABLE:
            ...
            *(sys_call_table + __NR_open) = (unsigned long)custom_open;
            real_creat = (void*)*(sys_call_table + __NR_creat);
            *(sys_call_table + __NR_creat) = (unsigned long)custom_creat;
            real_openat = (void*)*(sys_call_table + __NR_openat);
            *(sys_call_table + __NR_openat) = (unsigned long)custom_openat;
            make_ro((unsigned long)sys_call_table);
            is_set=1;
            printk(KERN_DEBUG "replaced the system call with custom call\n");
            break;
        case IOCTL_FIX_TABLE:
            make_rw((unsigned long)sys_call_table);
            *(sys_call_table + __NR_open) = (unsigned long)real_open;
            *(sys_call_table + __NR_creat) = (unsigned long)real_creat;
            *(sys_call_table + __NR_openat) = (unsigned long)real_openat;
            make_ro((unsigned long)sys_call_table);
            is_set=0;
            printk(KERN_DEBUG "restored the original system call\n");
            break;
        default:
            printk(KERN_DEBUG "unsupported ioctl code\n");
            break;
    }
